...
Table\ref{table:db1-comparison} compares the between best run of each algorithm based on the value of precision, recall, F1-score, learning time and prediction time.

\begin{table}[p]
\begin{center}
\begin{tabular}{|p{5cm}|c|c|c|p{2cm}|p{2cm}|}
\hline Algorithm & Precision & Recall & F1-score & Learning Time & Prediction Time \\
\hline Logistic Regression, PreProc1, C=0.1, Exponent=10 & 0.50 & 0.57 & 0.56 & &\\
\hline Decision Tree, PreProc1, Criterion=Gini & 0.48 & 0.48 & 0.48 & &\\
\hline K Nearest Neighbors, PreProc1, NN=30 & 0.57 & 0.56 & 0.56 & &\\
\hline SVM, PreProc1, Kernel=RBF, C=1 & \textbf{0.58} & \textbf{0.57} & \textbf{0.57} & &\\
\hline Neural Network, PreProc1, NL=1, NN=120 & 0.40 & 0.38 & 0.37 & &\\
\hline
\end{tabular}
...
\subsection{Imputation Experiment}