...
def Main():
	...
	_container = algorithmcontainer.Container(train[1], train[0], validation[1], validation[0], Constants.MISSING_VALUE_METHOD_NONE, range(0,57), Constants.MACHINE_LEARNING_METHOD_CLASSIFICATION,preprocessing_method)
	#_container.push(DecisionTree().SetAlgorithmName('DecisionTree'))
	_container.push(SupportVectorMachine().SetAlgorithmName('SVM_10_valid').ExtraParams(C=10))
